...
export class BaseService {
  ...
  protected get<T>(url: string): Promise<AxiosApiResponse<T>> {
    return instance.get<ApiResponse<T>>(`${this.apiBaseUrl}${url ? ('/' + url) : ''}`);
  }

  protected post<T>(url: string, data: any): Promise<AxiosApiResponse<T>> {
    return instance.post<ApiResponse<T>>(`${this.apiBaseUrl}${url ? ('/' + url) : ''}`, data);
  }

  protected put<T>(url: string, data: any): Promise<AxiosApiResponse<T>> {
    return instance.put<ApiResponse<T>>(`${this.apiBaseUrl}${url ? ('/' + url) : ''}`, data);
  }

  protected delete<T>(url: string): Promise<AxiosApiResponse<T>> {
    return instance.delete<ApiResponse<T>>(`${this.apiBaseUrl}${url ? ('/' + url) : ''}`);
  }
}